...
.guru-name { font-size: 13px; font-weight: 600; color: #334155; }
...
<div class="ct-wrap">
    ...
    <div class="ct-header">
        <div class="ct-header-left">
            ...
            <h1><i class="fas fa-arrow-right-from-bracket" style="font-size:24px; margin-right:12px; vertical-align:middle;"></i>Laporan Barang Keluar</h1>
            <div class="ct-header-sub">Riwayat transaksi barang keluar</div>
        </div>
                ...
                <table id="table_id" class="display">
                    <thead>
                        <tr>
                            ...
                            <th style="text-align:left; padding-left:16px;">Guru & Tendik</th>
                        </tr>
                    </thead>
                    <tbody id="tabel-laporan-barang-keluar"></tbody>
                </table>
            </div>
...
$(document).ready(function () {
    ...
    function loadData() {
        ...
        $.when(
            ...
            customerCache !== null
                ? $.Deferred().resolve(customerCache).promise()
                : $.getJSON('{{ url('api/customer') }}').then(function (customers) {
                    var map = {};
                    $.each(customers, function (i, c) { map[c.id] = c.customer; });
                    customerCache = map;
                    return map;
                  })
        ).then(function (dataResult, customersResult) {

            var response  = Array.isArray(dataResult)       ? dataResult[0]       : dataResult;
            var customers = Array.isArray(customersResult)  ? customersResult[0]  : customersResult;
            ...
            if (response.length > 0) {
                $.each(response, function (index, item) {
                    var customerName = customers[item.customer_id] || '-';

                    table.row.add([
                        `<div class="td-no">${index + 1}</div>`,
                        `<span class="kode-badge"><i class="fas fa-barcode"></i>${item.kode_transaksi}</span>`,
                        `<span class="tanggal-badge"><i class="fas fa-calendar-days"></i>${item.tanggal_keluar}</span>`,
                        `<div class="barang-cell">
                            <div class="barang-icon"><i class="fas fa-box-open"></i></div>
                            <div class="barang-name">${item.nama_barang}</div>
                        </div>`,
                        `<span class="badge-keluar"><i class="fas fa-arrow-up" style="font-size:11px"></i> ${item.jumlah_keluar}</span>`,
                        `<div class="guru-cell">
                            <div class="guru-avatar"><i class="fas fa-user"></i></div>
                            <div class="guru-name">${customerName}</div>
                        </div>`
                    ]);
                });
            }
    ...
    $('#download-excel').on('click', function () {
        window.location.href = '/laporan-barang-keluar/export-excel' + getExportParams();
    });

    $('#save-pdf').on('click', function () {
        window.location.href = '/laporan-barang-keluar/save-pdf' + getExportParams();
    });
