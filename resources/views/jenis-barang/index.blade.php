...
<style>
/* =============================================
   DATA JENIS BARANG — PREMIUM UPGRADE
============================================= */
.jb-wrap {
    font-family: 'Plus Jakarta Sans', sans-serif;
    padding: 4px 2px;
    animation: jbFadeIn .45s ease both;
}
...
.jb-header-left { position: relative; z-index: 1; }
...
.jb-header-eyebrow i { font-size: 9px; color: #93c5fd; }
...
.jb-stat-pill .spn { font-size: 22px; font-weight: 800; color: #fff; line-height: 1; }
.jb-stat-pill .spl { font-size: 10px; font-weight: 700; color: rgba(255,255,255,.6); text-transform: uppercase; letter-spacing: .5px; margin-top: 2px; }
...
#table_jenis_barang thead th:first-child { border-radius: 12px 0 0 12px !important; }
#table_jenis_barang thead th:last-child  { border-radius: 0 12px 12px 0 !important; }

#table_jenis_barang tbody tr {
    background: #fff;
    border-radius: 14px;
...
#table_jenis_barang tbody tr:hover {
...
#table_jenis_barang tbody td:first-child { border-radius: 14px 0 0 14px !important; }
#table_jenis_barang tbody td:last-child  { border-radius: 0 14px 14px 0 !important; }
...
<div class="jb-wrap">
    ...
    <div class="jb-header">
        <div class="jb-header-left">
            <div class="jb-header-eyebrow">
                <i class="fas fa-circle"></i> Manajemen Stok
            </div>
            <h1><i class="fas fa-tags" style="font-size:26px; margin-right:12px; vertical-align:middle;"></i>Data Kategori Barang</h1>
            <div class="jb-header-sub">Kelola kategori barang inventaris</div>
        </div>

        <div class="jb-header-right">
            <div class="jb-stat-pill">
                <div class="spn" id="statTotalJenis">—</div>
                <div class="spl">Total Kategori</div>
            </div>
            <a href="javascript:void(0)" class="btn-add-jenis" id="button_tambah_jenis_barang">
    ...
    <div class="jb-toolbar">
        <div class="jb-search-wrap">
            <i class="fas fa-search jb-search-icon"></i>
            <input type="text" id="jbSearch" placeholder="Cari kategori barang…">
        </div>
    </div>

    {{-- ── MAIN CARD ── --}}
    <div class="jb-card">
        <div class="jb-card-stripe"></div>
        <div class="jb-card-body">
            <div class="table-responsive">
                <table id="table_jenis_barang" class="display">
                    <thead>
                        <tr>
...
$(document).ready(function () {
    ...
    // ── INIT DATATABLE ──
    let table = $('#table_jenis_barang').DataTable({
        paging:   true,
        ordering: false,
        ...
        language: {
            ...
            info:       'Menampilkan _START_–_END_ dari _TOTAL_ jenis',
    ...
    // ── LOAD DATA ──
    function loadData() {
        $.get('/jenis-barang/get-data', function (res) {
            table.clear();
            let no = 1;
            $('#statTotalJenis').text(res.data.length);

            $.each(res.data, function (i, item) {
                let initial = item.jenis_barang.charAt(0).toUpperCase();

                table.row.add([
                    `<div class="td-no">${no++}</div>`,

                    `<div class="jenis-cell">
                        <div class="jenis-icon">${initial}</div>
                        <div class="jenis-name">${item.jenis_barang}</div>
                    </div>`,
    ...
    $('#store').click(function (e) {
        ...
        formData.append('jenis_barang', $('#jenis_barang').val());
        ...
        $.ajax({
            ...
            error: function (err) {
                if (err.responseJSON?.jenis_barang?.[0]) {
                    $('#alert-jenis_barang').removeClass('d-none').html(err.responseJSON.jenis_barang[0]);
                }
            }
        });
    });
    ...
    $('body').on('click', '#button_edit_jenis_barang', function () {
        let id = $(this).data('id');
        $.get(`/jenis-barang/${id}/edit`, function (res) {
    ...
    $('#update').click(function (e) {
        ...
        let id = $('#jenis_id').val();
        let formData = new FormData();
        formData.append('jenis_barang', $('#edit_jenis_barang').val());
        formData.append('_token',       $("meta[name='csrf-token']").attr('content'));
        formData.append('_method',      'PUT');

        $.ajax({
            url: `/jenis-barang/${id}`, type: 'POST',
            data: formData, contentType: false, processData: false,
            success: function (res) {
                playSound('success');
                Swal.fire({ icon: 'success', title: res.message, timer: 2500, showConfirmButton: false });
    ...
    $('body').on('click', '#button_hapus_jenis_barang', function () {
        ...
        Swal.fire({
            ...
            text:  'Data kategori barang ini akan dihapus permanen.',
            icon:  'warning',
